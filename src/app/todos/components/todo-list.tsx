...
function AddTodoForm({
  ...
  userLeads: Lead[];
  onAddLeadAndTask: (
    newLead: Omit<Lead, 'id' | 'createdAt' | 'ownerName'> & {
      initialNotes?: string;
    },
  ...
  const handleLeadCreated = (newLead: Lead) => {
    ...
    onAdd(`Seguimiento inicial para ${newLead.name}`, {
      id: newLead.id,
      name: newLead.name,
    });
  };
    ...
    >
      <CardContent className="p-3 flex items-center gap-3">
        <GripVertical
          size={16}
          className={cn(
            'text-slate-300 transition-opacity',
            todo.completed
              ? 'opacity-0'
        ...
        <div className="flex-1">
          ...
          <div className="text-xs text-slate-400 flex items-center gap-2 mt-1">
            <span>
              Added{' '}
              {formatDistanceToNow(
                (todo.createdAt as any)?.toDate() || new Date(),
                { addSuffix: true }
              )}
            </span>
        ...
        <Button
          ...
          className="text-destructive h-8 w-8 opacity-50 group-hover:opacity-100"
        >
          <Trash2 size={16} />
        </Button>
      </CardContent>
...
export function TodoList({
  ...
  const handleAddTodo = async (
    title: string,
    linkedLead?: { id: string; name: string }
  ) => {
  ...
  const handleAddLeadAndTask = useCallback(
    async (
      newLeadData: Omit<Lead, 'id' | 'createdAt' | 'ownerName'> & {
        initialNotes?: string;
      },
      callback: (lead: Lead) => void
    ) => {
      ...
      try {
        ...
        const noteHistoryRef = collection(
          firestore,
          'leads',
          newDocRef.id,
          'noteHistory'
        ...
        toast({
          title: 'Lead Added',
          description:
            'New lead created successfully. A task will be added.',
        });
        ...
        toast({
          ...
          description: 'Could not save the new lead.',
          variant: 'destructive',
        });
      }
    },
